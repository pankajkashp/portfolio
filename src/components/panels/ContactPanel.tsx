  ...
  return (
    <div className="space-y-16 py-10">
      <div className="space-y-3">
        <h3 className="text-xs font-black uppercase tracking-[0.5em] text-[#3b82f6]">Signal Uplink</h3>
        <p className="text-white/40 text-lg font-light">Secure channel for strategic collaboration.</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[1fr_1.5fr] gap-20">
        ...
        <div className="space-y-12">
          <div className="space-y-8">
            {[
              { icon: <Mail size={20} />, label: 'Relay Address', value: personalInfo.email, color: '#3b82f6' },
              { icon: <MapPin size={20} />, label: 'Neural Node', value: personalInfo.location, color: '#3b82f6' },
              { icon: <ShieldCheck size={20} />, label: 'Link Status', value: 'Ready for Transmission', color: '#22c55e' },
            ].map((item, i) => (
              <motion.div 
                key={item.label} 
          ...
          <div className="space-y-6">
             <h4 className="text-[10px] font-black uppercase tracking-[0.3em] text-white/20">Sync Channels</h4>
        ...
        <form onSubmit={handleSubmit} className="space-y-8 p-10 rounded-[3rem] bg-white/[0.02] border border-white/5 relative overflow-hidden">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            {[
              { label: 'Identifier', type: 'text', key: 'name' as const, placeholder: 'NAME' },
              { label: 'Relay', type: 'email', key: 'email' as const, placeholder: 'EMAIL@SECURE.COM' },
            ].map(f => (
          ...
          <div className="space-y-3">
            <label className="text-[10px] font-black uppercase tracking-[0.3em] text-white/20">Data Payload</label>
            <textarea 
              required 
              rows={5} 
              value={form.message} 
          ...
          <button 
            ...
            onMouseLeave={() => setCursorType('default')}
          >
            {sending ? 'TRANSMITTING...' : 'INITIATE UPLINK'} <Send size={18} />
          </button>
        </form>
      </div>
    </div>
